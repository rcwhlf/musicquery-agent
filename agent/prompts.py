...
# ---------- Step 1：意图判断 ----------
INTENT_PROMPT = """你是一个音乐数据库助手的守门员。请判断用户的输入是否与音乐数据库相关。

判定标准（宽松，宁可放行）：
- 只要输入中出现歌手、歌曲、专辑、流派、播放、用户行为等相关内容，即使只是一个名字（如"周杰伦"）或一个不完整的问句，都算相关，输出 YES；
- 只要能通过音乐库中的表（artists 歌手 / albums 专辑 / songs 歌曲 / users 用户 / play_records 播放记录）回答，就算相关，包括"共有多少首""有多少个"这类没有点名的泛化问句；
- 只有与音乐完全无关的领域（如天气、代码、数学题、闲聊问候等）才输出 NO。
...
5. 流派字段 genre 的合法取值只有：{genres}。用户口语中的分类必须映射到这些合法取值，可用 IN 覆盖相近取值（例如"流行歌曲"应写成 genre IN ('流行', '华语流行', '流行摇滚')）；
6. 若用户输入只是一个名字（歌手/歌曲/专辑名）而不是完整问句，默认按"查询与该名字相关的歌曲"生成 SQL（列出该歌手的歌曲或匹配该歌名的记录，加 LIMIT）；
7. 只输出一条 SQL 语句本身，不要任何解释、注释和 markdown 代码块。
